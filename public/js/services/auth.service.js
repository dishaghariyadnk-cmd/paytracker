...
class AuthService {
  ...
  logout(isExpired = false) {
    ...
    const redirectPath = window.location.pathname.includes('/public/views/') ? './logout.html' : './public/views/logout.html';
    if (isExpired) {
      window.location.href = redirectPath + '?reason=expired';
    } else {
      window.location.href = redirectPath + '?reason=user';
    }
  }

  requireAuth() {
    if (!this.isAuthenticated()) {
      const loginPath = window.location.pathname.includes('/public/views/') ? './login.html' : './public/views/login.html';
